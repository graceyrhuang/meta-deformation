...
class Inference(object):
    def __init__(self, HR=0, reg_num_steps=3000, model_path='trained_models/sup_human_network_last.pth', num_points=6890,
    ...
    def regress(self, points):
        ...
        while np.log(loss) > -9.5 and i < self.reg_num_steps:
            ...
            # pointsReconstructed = pointsReconstructed.permute([, 0])
            pointsReconstructed = pointsReconstructed.view(pointsReconstructed.size(0), -1, 3)
            # print(pointsReconstructed.shape)
            # print(points.transpose(2, 1).contiguous().shape)
    ...
    def run(self, input, scalefactor, path):
        ...
        best_template = 0
        bestLoss = 10
        pointsReconstructed, _ = self.network(points_LR)
        print(f'random sample: {random_sample.shape}, point:{points_LR.shape}, recons:{pointsReconstructed.shape}')
        # return shape[1, 3, # of template, # of points]
        pointsReconstructed = pointsReconstructed[:, :, [0], :].squeeze(2)
        pointsReconstructed = pointsReconstructed.permute([0, 2, 1])
        # print(type(pointsReconstructed))
        # print(points_LR.transpose(2, 1).contiguous().shape)
        pointsReconstructed = pointsReconstructed.view(pointsReconstructed.shape[0], -1, 3)
        ...
        for j in range(THETA.shape[1]):
            for i in range(THETA.shape[0]):
                ...
                pointsReconstructed, _ = self.network(input_network)
                pointsReconstructed = pointsReconstructed[:, :, [0], :].squeeze(2)
                pointsReconstructed = pointsReconstructed.permute([0, 2, 1])
                # print(pointsReconstructed.shape)
                # pointsReconstructed = pointsReconstructed.view(pointsReconstructed.size(0), -1, 3)
...
if __name__ == '__main__':
    ...
    # trainer.network.module.make_high_res_template_from_low_res()
